...
export const getActiveModules = catchAsync(async () => {
  // console.log("token :>> ", token);
  const res = await uarApi
    .get("/modules/active")
    .then((res) => res.data)
...
export const getShiftsByUser = catchAsync(async () => {
  const token = getToken();
  const res = await uarApi
    .get("/shifts/by-user/?onlyToday=true", {
      headers: {
        Authorization: `Bearer ${token}`,
        "Content-Type": "application/json",
      },
...
export const getTodayShiftsByUsers = catchAsync(async (date, module) => {
  const token = getToken();
  const res = await uarApi
...
export const getAttendingTimesByUser = catchAsync(async (date, module) => {
  const token = getToken();
  const res = await uarApi
